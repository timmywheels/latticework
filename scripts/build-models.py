...
def main():
    ...
    L.append(']')
    L.append('')
    import_srcs = os.path.join(PROJ, 'data', 'sources.json')
    if os.path.exists(import_srcs):
        SRC = json.load(open(import_srcs)).get('sources', [])
        L.append('export interface Source {')
        L.append('  id: string')
        L.append('  title: string')
        L.append('  creator: string')
        L.append('  host: string')
        L.append('  type: string')
        L.append('  year: string')
        L.append('  url: string')
        L.append('  supports: string')
        L.append('  attributionNote: string')
        L.append('}')
        L.append('')
        L.append('/** Whether a source is Munger\'s own words (primary) or a later compilation/reference. */')
        L.append("export const PRIMARY_SOURCE_TYPES = new Set([")
        L.append("  'primary-talk', 'primary-writing', 'primary-meeting-transcript', 'primary-interview',")
        L.append('])')
        L.append('')
        L.append('export const SOURCES: Record<string, Source> = {')
        for sc in SRC:
            fields = ', '.join(f"{k}: '{esc(sc.get(k, ''))}'" for k in
                               ['id','title','creator','host','type','year','url','supports','attributionNote'])
            L.append(f"  {sc['id']}: {{ {fields} }},")
        L.append('}')
        L.append('')
    ...
    L.append('}')
    L.append('')
    L.append('/** All models in plate-number order — the daily-email sequence reads from this. */')
    L.append('export const PLATE_ORDER: Model[] = BY_NUMBER')
    L.append('')
    L.append('// URL slugs, derived from names: /models/multiply-by-zero, not /models/M457.')
    L.append('// Collisions dedupe deterministically (-2, -3, …) in MODELS order.')
    L.append('function slugify(name: string): string {')
    L.append('  return name')
    L.append('    .toLowerCase()')
    L.append('    .normalize(\'NFKD\')')
    L.append('    .replace(/[\\u0300-\\u036f\'’]/g, \'\')')
    L.append('    .replace(/[^a-z0-9]+/g, \'-\')')
    L.append('    .replace(/^-+|-+$/g, \'\')')
    L.append('}')
    L.append('')
    L.append('export const MODEL_SLUGS: Record<string, string> = {}')
    L.append('export const MODELS_BY_SLUG: Record<string, Model> = {}')
    L.append('{')
    L.append('  const taken = new Set<string>()')
    L.append('  for (const m of MODELS) {')
    L.append('    let s = slugify(m.name) || m.id.toLowerCase()')
    L.append('    if (taken.has(s)) {')
    L.append('      let n = 2')
    L.append('      while (taken.has(`${s}-${n}`)) {')
    L.append('        n++')
    L.append('      }')
    L.append('      s = `${s}-${n}`')
    L.append('    }')
    L.append('    taken.add(s)')
    L.append('    MODEL_SLUGS[m.id] = s')
    L.append('    MODELS_BY_SLUG[s] = m')
    L.append('  }')
    L.append('}')
    L.append('')
    L.append('export function modelPath(m: Model): string {')
    L.append('  return `/models/${MODEL_SLUGS[m.id]}`')
    L.append('}')
    L.append('')
    L.append('// Everyone gets the same model on the same day — deterministic, no cursor to store.')
    L.append('// Matched to server/daily.ts so the site and the daily email always agree.')
    L.append('const LAUNCH_UTC_DAY = Math.floor(Date.UTC(2026, 6, 17) / 86_400_000)')
    L.append('')
    L.append('export function modelOfTheDay(now = new Date()): Model {')
    L.append('  const day = Math.floor(now.getTime() / 86_400_000)')
    L.append('  const n = PLATE_ORDER.length')
    L.append('  const idx = (((day - LAUNCH_UTC_DAY) % n) + n) % n')
    L.append('  return PLATE_ORDER[idx]')
    L.append('}')
    L.append('')
    L.append('/** A random model — the "surprise me" pick. */')
    L.append('export function randomModel(): Model {')
    L.append('  return MODELS[Math.floor(Math.random() * MODELS.length)]')
    L.append('}')
    L.append('')
